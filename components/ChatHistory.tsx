...
export default function ChatHistory({ onSelectChat, onNewChat }: ChatHistoryProps) {
  ...
  const handleExportChat = async (chatId: number, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    try {
      const result = await window.electronAPI.exportChatToPDF(chatId);
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1 ml-2">
                    <button
                      onClick={(e) => handleExportChat(session.id, e)}
                      disabled={exportingId === session.id}
                      className="p-1 text-gray-500 hover:text-green-600 dark:hover:text-green-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                      title="Export to PDF"
                    >
